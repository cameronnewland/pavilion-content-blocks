...
if (have_posts()) {
	...
	$prostart_classes = 'posts_container '
						. (substr(prostart_get_theme_option('blog_style'), 0, 7) == 'classic' ? 'columns_wrap columns_padding_bottom' : 'masonry_wrap');
	...
	if (!$prostart_sticky_out) {
		if (prostart_get_theme_option('first_post_large') && !is_paged() && !in_array(prostart_get_theme_option('body_style'), array('fullwide', 'fullscreen'))) {
			the_post();
			get_template_part( 'content', 'excerpt' );
		}
		
		?><div class="<?php echo esc_attr($prostart_classes); ?>"><?php
	}
	while ( have_posts() ) { the_post(); 
		if ($prostart_sticky_out && !is_sticky()) {
			$prostart_sticky_out = false;
			?></div><div class="<?php echo esc_attr($prostart_classes); ?>"><?php
		}
		get_template_part( 'content', $prostart_sticky_out && is_sticky() ? 'sticky' : 'classic' );
	}
