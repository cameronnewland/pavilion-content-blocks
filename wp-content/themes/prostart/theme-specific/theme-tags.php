...
if ( !function_exists('prostart_need_page_title') ) {
	function prostart_need_page_title() {
		return !is_front_page() && apply_filters('prostart_filter_need_page_title', true);
	}
}
...
if ( !function_exists('prostart_show_layout') ) {
	function prostart_show_layout($str, $before='', $after='') {
		if (trim($str) != '') {
...
if ( !function_exists('prostart_get_logo_image') ) {
	function prostart_get_logo_image($type='') {
		$logo_image = '';
		if (prostart_is_on(prostart_get_theme_option('logo_retina_enabled')) && prostart_get_retina_multiplier() > 1)
			$logo_image = prostart_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '').'_retina' );
		if (empty($logo_image)) {
			if (empty($type) && function_exists('the_custom_logo')) {
				...
				$logo_image = prostart_get_theme_option( 'logo'.(!empty($type) ? '_'.trim($type) : '') );
			}
		}
		return prostart_remove_protocol_from_url($logo_image, false);
	}
}

// Return header video (if set)
if ( !function_exists('prostart_get_header_video') ) {
	function prostart_get_header_video() {
		$video = '';
		if (apply_filters('prostart_header_video_enable', !wp_is_mobile() && is_front_page())) {
			if (prostart_check_theme_option('header_video')) {
				$video = prostart_get_theme_option('header_video');
...
if ( !function_exists('prostart_show_post_meta') ) {
	function prostart_show_post_meta($args=array()) {

		if (is_single() && prostart_is_off(prostart_get_theme_option('show_post_meta'))) return ' ';	// Space is need!
		...
		?><div class="post_meta"><?php
			...
			foreach ($components as $comp) {
				...
				if ($comp == 'categories') {
					...
					$cats = get_post_type()=='post' ? get_the_category_list(', ') : apply_filters('prostart_filter_get_post_categories', '');
					if (!empty($cats)) {
						?>
						<span class="post_meta_item post_categories"><?php prostart_show_layout($cats); ?></span>
						<?php
					}

				// Post date
				} else if ($comp == 'date') {
					$dt = apply_filters('prostart_filter_get_post_date', prostart_get_date());
					if (!empty($dt)) {
						?>
				...
				// Post counters
					...
					if ( ($output = prostart_get_post_counters($args['counters'])) != '')
						prostart_show_layout($output);
					else {
						if (!is_singular() || have_comments() || comments_open()) {
							...
							<a href="<?php echo esc_url(get_comments_link()); ?>" class="post_meta_item post_counters_item post_counters_comments icon-comment-light"><?php
								...
								?><span class="post_counters_label"><?php
									echo esc_html(_n('Comment', 'Comments', $post_comments, 'prostart'));
								?></span>
							</a>
							<?php
						}
				...
				// Socials share
					...
					prostart_show_share_links(array(
							'type' => 'drop',
							'caption' => esc_html__('Share', 'prostart'),
				...
				// Edit page link
					...
					//edit_post_link( esc_html__( 'Edit', 'prostart' ), '<span class="post_meta_item post_edit">', '</span>' );
					edit_post_link( esc_html__( 'Edit', 'prostart' ), '', '', 0, 'post_meta_item post_edit icon-pencil' );
				}
			}
		?></div><!-- .post_meta --><?php
...
// Show post meta block: post date, author, categories, counters, etc.
if ( !function_exists('prostart_get_post_meta_array') ) {
    function prostart_get_post_meta_array($args=array()) {
        ...
        if ((strpos($args['components'], 'share')) !== false) {
            $res['share'] = prostart_get_share_links(array(
                            'type' => 'drop',
                            'echo' => false,
                            'caption' => esc_html__('Share', 'prostart'),
                            'before' => '<span class="post_meta_item post_share">',
        ...
        if ((strpos($args['components'], 'counters')) !== false) {
            ...
            if ((strpos($args['counters'], 'views')) !== false) {
                $res['views'] = prostart_get_post_counters('views');
                $res['counters'] .= $res['views'];
            }
            // Likes
            if ((strpos($args['counters'], 'likes')) !== false) {
                $res['likes'] = prostart_get_post_counters('likes');
                $res['counters'] .= $res['likes'];
            }
            // Comments
            if ((strpos($args['counters'], 'comments')) !== false) {
                $res['comments'] = prostart_get_post_counters('comments');
            ...
            if (empty($res['counters'])) {
                if (!is_singular() || have_comments() || comments_open()) {
                    ...
                    $res['comments'] = '<a href="' . esc_url(get_comments_link()) . '" class="post_meta_item post_counters_item post_counters_comments icon-comment-light">'
                                        ...
                                        . '<span class="post_counters_label">' . esc_html(_n('Comment', 'Comments', $post_comments, 'prostart')) . '</span></a>';
                }
            }
        }
        ...
        if ((strpos($args['components'], 'edit')) !== false) {
            $res['edit'] =     '<a class="post_meta_item post_edit icon-pencil" href="' . get_edit_post_link() . '">' . esc_html__( 'Edit', 'prostart' ) . '</a>';
        }
...
if ( !function_exists('prostart_show_post_featured') ) {
	function prostart_show_post_featured($args=array()) {
//echo $args['post_info'];
		$args = array_merge(array(
			'hover' => prostart_get_theme_option('image_hover'),	// Hover effect
		...
		$thumb_size = !empty($args['thumb_size']) 
						? $args['thumb_size'] 
						: prostart_get_thumb_size(is_attachment() || is_single() ? 'full' : 'big');
		$post_format = str_replace('post-format-', '', get_post_format());
		$no_image = !empty($args['show_no_image']) ? prostart_get_no_image() : '';
		if ($args['thumb_bg']) {
			if (has_post_thumbnail()) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), $thumb_size );
				$image = $image[0];
			} else if ($post_format == 'image') {
				$image = prostart_get_post_image();
				if (!empty($image)) 
					$image = prostart_add_thumb_size($image, $thumb_size);
			}
			if (empty($image))
				$image = $no_image;
			if (!empty($image))
				$args['class'] .= ($args['class'] ? ' ' : '') . 'post_featured_bg' . ' ' . prostart_add_inline_css_class('background-image: url('.esc_url($image).');');
		}
			...
			if ( is_attachment() ) {
				...
				<div class="post_featured post_attachment<?php if ($args['class']) echo ' '.esc_attr($args['class']); ?>">
					...
					if (!$args['thumb_bg']) 
						echo wp_get_attachment_image( get_the_ID(), $thumb_size, false, 
													 prostart_is_on(prostart_get_theme_option('seo_snippets'))
				...
				?>>
					...
					if (has_post_thumbnail() && $args['seo']) {
						$prostart_attr = prostart_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
						?>
						<meta itemprop="width" content="<?php echo esc_attr($prostart_attr[0]); ?>">
						<meta itemprop="height" content="<?php echo esc_attr($prostart_attr[1]); ?>">
						<?php
					}
					if (!$args['thumb_bg']) {
						if ( has_post_thumbnail() ) {
					...
					prostart_show_layout($post_info);
					?>
                </div><!-- .post_featured -->
				<?php
			}
			...
			if ($has_thumb 
				...
				if ( $args['thumb_bg'] ) {
					...
					if (!in_array($post_format, array('audio', 'video'))) {
						prostart_hovers_add_icons($args['hover']);
					}
					...
					if (($output=prostart_get_slider_layout(array('thumb_size'=>$thumb_size, 'controls'=>'yes', 'pagination'=>'yes'))) != '')
						prostart_show_layout($output);
					...
					if (!empty($image)) {
						...
						prostart_hovers_add_icons($args['hover'], array('image' => $image));
					}
				...
				if (!$args['thumb_only'] && ($post_format == 'video' || $post_format == 'audio')) {
					...
					if ($post_format == 'video') {
						$video = prostart_get_post_video($post_content, false);
						if (empty($video))
							$video = prostart_get_post_iframe($post_content, false);
						if (empty($video)) {
							...
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($videos) && is_array($videos)) {
									$video = prostart_array_get_first($videos, false);
								}
							}
						}
						if (!empty($video)) {
							if ( $has_thumb ) {
								$video = prostart_make_video_autoplay($video);
								?><div class="post_video_hover" data-video="<?php echo esc_attr($video); ?>"></div><?php 
							}
							?><div class="post_video video_frame"><?php 
								if ( !$has_thumb ) {
									prostart_show_layout($video);
								}
							?></div><?php
						}
					...
					// Put audio over the thumb
						...
						$audio = prostart_get_post_audio($post_content, false);
						if (empty($audio))
							$audio = prostart_get_post_iframe($post_content, false);
						if (empty($audio)) {
							// Only get video from the content if a playlist isn't present.
							$post_content = apply_filters( 'the_content', get_the_content() );
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($audios) && is_array($audios)) {
									$audio = prostart_array_get_first($audios, false);
								}
							}
						}
						if (!empty($audio)) {
							?><div class="post_audio<?php if (strpos($audio, 'soundcloud')!==false) echo ' with_iframe'; ?>"><?php 
								...
								$media_author = prostart_get_theme_option('media_author', '', false, get_the_ID());
								$media_title = prostart_get_theme_option('media_title', '', false, get_the_ID());
								if ( !empty($media_author) && !prostart_is_inherit($media_author) ) {
									?><div class="post_audio_author"><?php prostart_show_layout($media_author); ?></div><?php
								}
								if ( !empty($media_title) && !prostart_is_inherit($media_title) ) {
									?><h5 class="post_audio_title"><?php prostart_show_layout($media_title); ?></h5><?php
								}
								// Display audio
								prostart_show_layout($audio); 
							?></div><?php
						}
					}
				}
...
if ( !function_exists('prostart_get_no_image') ) {
	function prostart_get_no_image($no_image='') {
		static $img = '';
		if (empty($img)) {
			$img = prostart_get_theme_option( 'no_image' );
			if (empty($img)) $img = prostart_get_file_url('images/no-image.jpg');
		}
...
if ( !function_exists('prostart_add_bg_in_post_nav') ) {
	function prostart_add_bg_in_post_nav() {
		if ( ! is_single() ) return;
	
		$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
		$next     = get_adjacent_post( false, '', false );
		$css      = '';
		$noimg    = prostart_get_no_image();
		
		if ( is_attachment() && $previous->post_type == 'attachment' ) return;
	
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), prostart_get_thumb_size('med') );
				$img = $img[0];
			} else if (prostart_get_theme_setting('allow_no_image'))
				$img = $noimg;
			if ( !empty($img) )
				$css .= '.post-navigation .nav-previous a .nav-arrow { background-image: url(' . esc_url( $img ) . '); }';
			else
				$css .= '.post-navigation .nav-previous a .nav-arrow { background-color: rgba(128,128,128,0.05);}'
					 .  '.post-navigation .nav-previous a .nav-arrow:after { top: 0; opacity: 1; }';
		}
	
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), prostart_get_thumb_size('med') );
				$img = $img[0];
			} else if (prostart_get_theme_setting('allow_no_image'))
				$img = $noimg;
			if ( !empty($img) )
		...
		wp_add_inline_style( 'prostart-main', $css );
	}
}

// Show related posts
if ( !function_exists('prostart_show_related_posts') ) {
	function prostart_show_related_posts($args=array(), $style=1, $title='') {
		$args = array_merge(array(
...
//			'suppress_filters' => true,
		...
		$taxonomy = $args['post_type'] == 'post' ? 'category' : prostart_get_post_type_taxonomy();
			...
			<section class="related_wrap">
				<h3 class="section_title related_wrap_title"><?php
					if (!empty($title))
						...
						esc_html_e('You May Also Like', 'prostart');
				?></h3>
...
if ( !function_exists('prostart_show_portfolio_posts') ) {
	function prostart_show_portfolio_posts($args=array()) {
		$args = array_merge(array(
			'cat' => 0,
			'parent_cat' => 0,
			'taxonomy' => 'category',
		...
		$blog_style = explode('_', empty($args['blog_style']) ? prostart_get_theme_option('blog_style') : $args['blog_style']);
		$style = $blog_style[0];
		...
		if ( !$args['echo'] ) {
			...
			$q_args = prostart_query_add_posts_and_cats($q_args, '', $args['post_type'], $args['cat'], $args['taxonomy']);
			if ($args['page'] > 1) {
				$q_args['paged'] = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = prostart_get_theme_option('posts_per_page');
			if ((int) $ppp != 0)
...
// AJAX handler for the prostart_ajax_get_posts action
if ( !function_exists( 'prostart_ajax_get_posts_callback' ) ) {
	add_action('wp_ajax_prostart_ajax_get_posts',			'prostart_ajax_get_posts_callback');
	add_action('wp_ajax_nopriv_prostart_ajax_get_posts',	'prostart_ajax_get_posts_callback');
	function prostart_ajax_get_posts_callback() {
		...
		if ($id > 0) {
			...
			prostart_storage_set('blog_mode', 'blog');
			prostart_storage_set('options_meta', get_post_meta($id, 'prostart_options', true));
		}

		$response = array(
			'error'=>'', 
			'data' => prostart_show_portfolio_posts(array(
		...
		if (empty($response['data'])) {
			$response['error'] = esc_html__('Sorry, but nothing matched your search criteria.', 'prostart');
		}
...
// Show pagination
if ( !function_exists('prostart_show_pagination') ) {
	function prostart_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = prostart_get_theme_option('blog_pagination');
		if ($pagination == 'pages') {
			the_posts_pagination( array(
				'mid_size'  => 2,
				'prev_text' => esc_html__( '<', 'prostart' ),
				'next_text' => esc_html__( '>', 'prostart' ),
				'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'prostart' ) . ' </span>',
			) );
			...
			if ($page_number < $wp_query->max_num_pages) {
				...
				<div class="nav-links-more<?php if ($pagination == 'infinite') echo ' nav-links-infinite'; ?>">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e('Load more posts', 'prostart'); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__('Previous posts', 'prostart') : esc_html__('Newest posts', 'prostart') ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__('Next posts', 'prostart') : esc_html__('Older posts', 'prostart'), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
