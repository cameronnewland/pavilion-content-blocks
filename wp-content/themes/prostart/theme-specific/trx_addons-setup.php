...
------------------------------------------------------------------------------- */
if (!function_exists('prostart_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'prostart_trx_addons_theme_specific_components');
	function prostart_trx_addons_theme_specific_components($enable=false) {
		return false;
	}
}

if (!function_exists('prostart_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'prostart_trx_addons_theme_specific_setup1', 1 );
	function prostart_trx_addons_theme_specific_setup1() {
		if (prostart_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',					'prostart_trx_addons_cv_enable');
			add_filter( 'trx_addons_demo_enable',				'prostart_trx_addons_demo_enable');
			add_filter( 'trx_addons_filter_edd_themes_market',	'prostart_trx_addons_edd_themes_market_enable');
			add_filter( 'trx_addons_cpt_list',					'prostart_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',					'prostart_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',				'prostart_trx_addons_widgets_list');
		}
	}
}

// CV
if ( !function_exists( 'prostart_trx_addons_cv_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_cv_enable', 'prostart_trx_addons_cv_enable');
	function prostart_trx_addons_cv_enable($enable=false) {
		// To do: return false if theme not use CV functionality
		return PROSTART_THEME_FREE
					? false		// Free version
					: true;		// Pro version
	}
}
...
// Demo mode
if ( !function_exists( 'prostart_trx_addons_demo_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_demo_enable', 'prostart_trx_addons_demo_enable');
	function prostart_trx_addons_demo_enable($enable=false) {
...
// EDD Themes market
if ( !function_exists( 'prostart_trx_addons_edd_themes_market_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_edd_themes_market', 'prostart_trx_addons_edd_themes_market_enable')) {
	function prostart_trx_addons_edd_themes_market_enable($enable=false) {
...
// API
if ( !function_exists( 'prostart_trx_addons_api_list' ) ) {
	//Handler of the add_filter('trx_addons_api_list',	'prostart_trx_addons_api_list');
	function prostart_trx_addons_api_list($list=array()) {
...
// CPT
if ( !function_exists( 'prostart_trx_addons_cpt_list' ) ) {
	//Handler of the add_filter('trx_addons_cpt_list',	'prostart_trx_addons_cpt_list');
	function prostart_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'prostart_trx_addons_sc_list' ) ) {
	//Handler of the add_filter('trx_addons_sc_list',	'prostart_trx_addons_sc_list');
	function prostart_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'prostart_trx_addons_widgets_list' ) ) {
	//Handler of the add_filter('trx_addons_widgets_list',	'prostart_trx_addons_widgets_list');
	function prostart_trx_addons_widgets_list($list=array()) {
...
if ( !function_exists( 'prostart_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'prostart_trx_addons_menu_cache');
	function prostart_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('prostart_trx_addons_localize_script')) {
	...
	function prostart_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = prostart_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'prostart_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'prostart_trx_addons_sc_type', 10, 2);
...
if ( !function_exists( 'prostart_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'prostart_trx_addons_sc_atts', 10, 2);
	function prostart_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'prostart_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'prostart_trx_addons_sc_map', 10, 2);
	function prostart_trx_addons_sc_map($params, $sc) {
            ...
            foreach ($aa as $k => $v) {
                if($v['param_name'] == 'featured'){
                    $params['params'][$k]['dependency'] = array(
                        'element' => 'type',
                        'value' => array('default', 'light', 'iconed', 'callouts', 'list', 'hover', 'chess', 'timeline', 'tabs', 'tabs_simple')
                    );
                }
                if($v['param_name'] == 'hide_excerpt'){
                    $params['params'][$k]['dependency'] = array(
                        'element' => 'type',
                        'value' => array('default', 'light', 'extra', 'iconed', 'callouts', 'list', 'hover', 'chess', 'timeline', 'tabs', 'tabs_simple')
                    );
                }
                ...
                if($v['param_name'] == 'popup'){
                    $params['params'][$k]['dependency'] = array(
                        'element' => 'type',
                        'value' => array('default', 'light', 'iconed', 'extra', 'callouts', 'list', 'hover', 'chess', 'timeline', 'tabs', 'tabs_simple')
                    );
                }
                if($v['param_name'] == 'no_margin'){
                    $params['params'][$k]['dependency'] = array(
                        'element' => 'type',
                        'value' => array('default', 'extra', 'light', 'iconed', 'callouts', 'list', 'hover', 'chess', 'timeline', 'tabs', 'tabs_simple')
                    );
                }
        ...
        if (in_array($sc, array('trx_sc_button'))) {
            ...
            foreach ($aa as $k => $v) {
                ...
                if($v['param_name'] == 'title'){
                    $params['params'][$k]['dependency'] = array(
                        'element' => 'type',
                        'value' => array('default', 'bordered', 'simple')
                    );
                }
        ...
        if (in_array($sc, array('trx_sc_googlemap'))) {
            ...
            $params['params'][] = array(
                'param_name' => 'map_shadow',
                'heading' => esc_html__('Shadow for the map', 'prostart'),
                'description' => wp_kses_data(__('Add a shadow to the map', 'prostart')),
                'edit_field_class' => 'vc_col-sm-6',
                'std' => "0",
                "value" => array(esc_html__("Yes", 'prostart') => "1"),
                "type" => "checkbox"
            );
        }
		...
		$param = array(
			...
			'heading' => esc_html__('Color style', 'prostart'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'prostart') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array_flip(prostart_get_list_sc_color_styles()),
			'type' => 'dropdown'
		);
		if (in_array($sc, array('trx_sc_button'))) {
			if (empty($params['params']) || !is_array($params['params'])) $params['params'] = array();
...
if ( !function_exists( 'prostart_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'prostart_trx_addons_sow_map', 10, 2);
	function prostart_trx_addons_sow_map($params, $sc) {

		// Param 'color_style'
		$param = array(
			'color_style' => array(
				'label' => esc_html__('Color style', 'prostart'),
				'description' => wp_kses_data( __('Select color style to decorate this block', 'prostart') ),
				'options' => prostart_get_list_sc_color_styles(),
				'default' => 'default',
				'type' => 'select'
			)
		);
		if (in_array($sc, array('trx_sc_button')))
			prostart_array_insert_after($params, 'size', $param);
		else if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
								'trx_sc_events', 'trx_sc_form',	'trx_sc_googlemap', 'trx_sc_portfolio', 'trx_sc_price', 'trx_sc_promo',
								'trx_sc_properties', 'trx_sc_services', 'trx_sc_team', 'trx_sc_testimonials', 'trx_sc_title',
								'trx_widget_audio', 'trx_widget_twitter')))
			prostart_array_insert_after($params, 'title_align', $param);
		return $params;
	}
}

// Add classes to the shortcode's output
if ( !function_exists( 'prostart_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'prostart_trx_addons_sc_output', 10, 4);
	function prostart_trx_addons_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_action'))) {
			...
			if (!empty($atts['color_style']) && !prostart_is_inherit($atts['color_style']))
...
if ( !function_exists( 'prostart_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'prostart_trx_addons_sc_item_title_tag');
	function prostart_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'prostart_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'prostart_trx_addons_sc_item_button_args', 10, 3);
	function prostart_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
if ( !function_exists( 'prostart_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'prostart_trx_addons_slider_title', 10, 2 );
	function prostart_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'prostart_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'prostart_trx_addons_sc_googlemap_styles');
	function prostart_trx_addons_sc_googlemap_styles($list) {
		$list['dark'] = esc_html__('Dark', 'prostart');
		return $list;
	}
}
...
if ( !function_exists( 'prostart_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'prostart_trx_addons_tiny_mce_style_formats');
	function prostart_trx_addons_tiny_mce_style_formats($style_formats) {
		...
		if (false && is_array($style_formats) && count($style_formats)>0 ) {
			foreach ($style_formats as $k=>$v) {
				if ( $v['title'] == esc_html__('List styles', 'prostart') ) {
					$style_formats[$k]['items'][] = array(
								'title' => esc_html__('Arrow', 'prostart'),
								'selector' => 'ul',
...
if ( !function_exists( 'prostart_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'prostart_filter_allow_override_header_image', 'prostart_trx_addons_allow_override_header_image' );
	function prostart_trx_addons_allow_override_header_image($allow) {
		return prostart_is_team_page() ? false : $allow;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'prostart_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'prostart_trx_addons_thumb_size', 10, 2);
	function prostart_trx_addons_thumb_size($thumb_size='', $type='') {
		if ($type == 'team-default')
			$thumb_size = prostart_get_thumb_size('card');
		return $thumb_size;
	}
}
...
if (!function_exists('prostart_trx_addons_meta_box_fields')) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'prostart_trx_addons_meta_box_fields', 10, 2);
	function prostart_trx_addons_meta_box_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'prostart'),
				"desc" => wp_kses_data( __("Team member's email", 'prostart') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
