...
if (!function_exists('prostart_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'prostart_hovers_theme_setup3', 3 );
	function prostart_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		prostart_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'prostart'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'prostart') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'prostart'),
//					'slide_left'	=> esc_html__('Slide from Left',	'prostart'),
//					'slide_right'	=> esc_html__('Slide from Right',	'prostart'),
//					'slide_top'		=> esc_html__('Slide from Top',		'prostart'),
//					'slide_bottom'	=> esc_html__('Slide from Bottom',	'prostart'),
//					'arrow'			=> esc_html__('Arrow',				'prostart'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'prostart'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'prostart') ),
				"std" => 'simple',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'prostart'),
                    'simple'	=> esc_html__('Simple',	'prostart'),
//					'icon'	=> esc_html__('Icon',	'prostart'),
//					'icons'	=> esc_html__('Icons',	'prostart'),
//					'zoom'	=> esc_html__('Zoom',	'prostart'),
//					'fade'	=> esc_html__('Fade',	'prostart'),
//					'slide'	=> esc_html__('Slide',	'prostart'),
//					'pull'	=> esc_html__('Pull',	'prostart'),
//					'border'=> esc_html__('Border',	'prostart')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('prostart_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'prostart_hovers_theme_setup9', 9 );
	function prostart_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',			'prostart_hovers_frontend_scripts', 1010 );
		add_filter( 'prostart_filter_localize_script','prostart_hovers_localize_script' );
		add_filter( 'prostart_filter_merge_scripts',	'prostart_hovers_merge_scripts' );
		add_filter( 'prostart_filter_merge_styles',	'prostart_hovers_merge_styles' );
		add_filter( 'prostart_filter_merge_styles_responsive', 'prostart_hovers_merge_styles_responsive' );
		add_filter( 'prostart_filter_get_css', 		'prostart_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'prostart_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'prostart_hovers_frontend_scripts', 1010 );
	function prostart_hovers_frontend_scripts() {
		if ( prostart_is_on(prostart_get_theme_option('debug_mode')) && prostart_get_file_dir('theme-specific/theme-hovers/theme-hovers.js')!='' )
			wp_enqueue_script( 'prostart-hovers', prostart_get_file_url('theme-specific/theme-hovers/theme-hovers.js'), array('jquery'), null, true );
	}
}

// Merge hover effects into single css
if (!function_exists('prostart_hovers_merge_styles')) {
	//Handler of the add_filter( 'prostart_filter_merge_styles', 'prostart_hovers_merge_styles' );
	function prostart_hovers_merge_styles($list) {
...
if (!function_exists('prostart_hovers_merge_styles_responsive')) {
	//Handler of the add_filter( 'prostart_filter_merge_styles_responsive', 'prostart_hovers_merge_styles_responsive' );
	function prostart_hovers_merge_styles_responsive($list) {
...
if (!function_exists('prostart_hovers_localize_script')) {
	//Handler of the add_filter( 'prostart_filter_localize_script','prostart_hovers_localize_script' );
	function prostart_hovers_localize_script($arr) {
		$arr['button_hover'] = prostart_get_theme_option('button_hover');
		return $arr;
	}
}

// Merge hover effects into single js
if (!function_exists('prostart_hovers_merge_scripts')) {
	//Handler of the add_filter( 'prostart_filter_merge_scripts', 'prostart_hovers_merge_scripts' );
	function prostart_hovers_merge_scripts($list) {
...
if ( !function_exists('prostart_hovers_add_icons') ) {
	function prostart_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat'])) {
					prostart_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? ($product->is_type( 'variable' ) ? esc_html__('Select options', 'prostart') : esc_html__('Buy now', 'prostart')) : '')
										. '</a>',
										$product));
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'prostart');
						else
							esc_html_e('Details', 'prostart');
					}
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'prostart_hovers_get_css' ) ) {
	//Handler of the add_filter( 'prostart_filter_get_css', 'prostart_hovers_get_css', 10, 4 );
	function prostart_hovers_get_css($css, $colors, $fonts, $scheme='') {
