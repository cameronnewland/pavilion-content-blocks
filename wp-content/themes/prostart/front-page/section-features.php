<div class="front_page_section front_page_section_features<?php
			$prostart_scheme = prostart_get_theme_option('front_page_features_scheme');
			if (!prostart_is_inherit($prostart_scheme)) echo ' scheme_'.esc_attr($prostart_scheme);
			echo ' front_page_section_paddings_'.esc_attr(prostart_get_theme_option('front_page_features_paddings'));
		?>"<?php
		$prostart_css = '';
		$prostart_bg_image = prostart_get_theme_option('front_page_features_bg_image');
...
?>><?php
	// Add anchor
	$prostart_anchor_icon = prostart_get_theme_option('front_page_features_anchor_icon');	
	$prostart_anchor_text = prostart_get_theme_option('front_page_features_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_features_inner<?php
			if (prostart_get_theme_option('front_page_features_fullheight'))
				echo ' prostart-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$prostart_css = '';
			$prostart_bg_mask = prostart_get_theme_option('front_page_features_bg_mask');
			$prostart_bg_color = prostart_get_theme_option('front_page_features_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$prostart_caption = prostart_get_theme_option('front_page_features_caption');
			if (!empty($prostart_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo !empty($prostart_caption) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post($prostart_caption); ?></h2><?php
			}
		
			// Description (text)
			$prostart_description = prostart_get_theme_option('front_page_features_description');
			if (!empty($prostart_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo !empty($prostart_description) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post(wpautop($prostart_description)); ?></div><?php
			}
			...
			?><div class="front_page_section_output front_page_section_features_output"><?php 
				if (is_active_sidebar('front_page_features_widgets')) {
					...
					if (!prostart_exists_trx_addons())
						...
						prostart_customizer_need_widgets_message('front_page_features_caption', 'ThemeREX Addons - Services');
				}
			?></div>
		</div>
	</div>
