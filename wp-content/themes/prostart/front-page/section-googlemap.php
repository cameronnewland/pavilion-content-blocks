<div class="front_page_section front_page_section_googlemap<?php
			$prostart_scheme = prostart_get_theme_option('front_page_googlemap_scheme');
			if (!prostart_is_inherit($prostart_scheme)) echo ' scheme_'.esc_attr($prostart_scheme);
			echo ' front_page_section_paddings_'.esc_attr(prostart_get_theme_option('front_page_googlemap_paddings'));
		?>"<?php
		$prostart_css = '';
		$prostart_bg_image = prostart_get_theme_option('front_page_googlemap_bg_image');
...
?>><?php
	// Add anchor
	$prostart_anchor_icon = prostart_get_theme_option('front_page_googlemap_anchor_icon');	
	$prostart_anchor_text = prostart_get_theme_option('front_page_googlemap_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_googlemap_inner<?php
			if (prostart_get_theme_option('front_page_googlemap_fullheight'))
				echo ' prostart-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$prostart_css = '';
			$prostart_bg_mask = prostart_get_theme_option('front_page_googlemap_bg_mask');
			$prostart_bg_color = prostart_get_theme_option('front_page_googlemap_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_googlemap_content_wrap<?php
			$prostart_layout = prostart_get_theme_option('front_page_googlemap_layout');
			if ($prostart_layout != 'fullwidth')
				echo ' content_wrap';
		?>">
			<?php
			// Content wrap with title and description
			$prostart_caption = prostart_get_theme_option('front_page_googlemap_caption');
			$prostart_description = prostart_get_theme_option('front_page_googlemap_description');
			if (!empty($prostart_caption) || !empty($prostart_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				if ($prostart_layout == 'fullwidth') {
					...
					if (!empty($prostart_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
						?><h2 class="front_page_section_caption front_page_section_googlemap_caption front_page_block_<?php echo !empty($prostart_caption) ? 'filled' : 'empty'; ?>"><?php
					...
					if (!empty($prostart_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
						?><div class="front_page_section_description front_page_section_googlemap_description front_page_block_<?php echo !empty($prostart_description) ? 'filled' : 'empty'; ?>"><?php
			...
			// Content (text)
			$prostart_content = prostart_get_theme_option('front_page_googlemap_content');
			if (!empty($prostart_content) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				if ($prostart_layout == 'columns') {
					?><div class="front_page_section_columns front_page_section_googlemap_columns columns_wrap">
						<div class="column-1_3">
					<?php
				} else if ($prostart_layout == 'fullwidth') {
					?><div class="content_wrap"><?php
				}
	
				?><div class="front_page_section_content front_page_section_googlemap_content front_page_block_<?php echo !empty($prostart_content) ? 'filled' : 'empty'; ?>"><?php
			...
			?><div class="front_page_section_output front_page_section_googlemap_output"><?php 
				if (is_active_sidebar('front_page_googlemap_widgets')) {
					...
					if (!prostart_exists_trx_addons())
						...
						prostart_customizer_need_widgets_message('front_page_googlemap_caption', 'ThemeREX Addons - Google map');
				}
			?></div><?php
