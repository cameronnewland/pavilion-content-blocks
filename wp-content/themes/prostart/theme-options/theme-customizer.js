/* global prostart_color_schemes, prostart_dependencies, Color */
...
( function( api ) {
	...
	for (var i in prostart_color_schemes) {
		cssTemplate[i] = wp.template( 'prostart-color-scheme-'+i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'prostart-fonts' );
	
	// Set initial state of controls
	api.bind('ready', function() {

		// Add 'reset' button
		jQuery('#customize-header-actions #save').before('<input type="button" class="button customize-action-reset" value="'+prostart_customizer_vars['msg_reset']+'">');
		jQuery('#customize-header-actions .customize-action-reset').on('click', function(e) {
			if (confirm(prostart_customizer_vars['msg_reset_confirm'])) {
		...
		if (prostart_customizer_vars && prostart_customizer_vars['theme_name_suffix'])
			jQuery('#customize-info .site-title').append(prostart_customizer_vars['theme_name_suffix']);

		// Blur the "load fonts" fields - regenerate options lists in the font-family controls
		jQuery('#customize-theme-controls [id^="customize-control-load_fonts"]').on('focusout', prostart_customizer_update_load_fonts);		

		// Click on the actions button
		jQuery('#customize-theme-controls .control-section .customize-control-button input[type="button"]').on('click', prostart_customizer_actions);

		// Check dependencies in the each section on ready
		jQuery('#customize-theme-controls .control-section').each(function () {
			prostart_customizer_check_dependencies(jQuery(this));
		});
		...
		jQuery('#customize-theme-controls .control-section > .accordion-section-title').on('click', function() {
			...
			if (id!='') {
				...
				if (section.length > 0) prostart_customizer_check_dependencies(section);
			}
		});
	...
	api.bind('change', function(obj) {
		prostart_customizer_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		prostart_customizer_refresh_preview(obj);
	});

	// Return value of the control
	function prostart_customizer_get_field_value(fld) {
	...
	function prostart_customizer_check_dependencies(cont) {
		cont.find('.customize-control').each(function() {
			...
			var fld=null, val='', i;
			var depend = false;
			for (fld in prostart_dependencies) {
				if (fld == id) {
					depend = prostart_dependencies[id];
					break;
				}
			}
			if (depend) {
				...
				for (i in depend) {
					...
					if (fld.length > 0) {
						val = prostart_customizer_get_field_value(fld);
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					ctrl.show().removeClass('prostart_options_no_use');
				} else {
					ctrl.hide().addClass('prostart_options_no_use');
				}
			}
			...
			if (id == 'color_scheme') {		// Disable color schemes less then main scheme!
				...
				if (fld.length > 0) {
					val = prostart_customizer_get_field_value(fld);
					var num = 0;
					for (i in prostart_color_schemes) {
						num++;
						if (i == val) break;
					}
					cont.find('.customize-control').each(function() {
	...
	function prostart_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			// Any theme fonts parameter is changed
				...
				for (opt in prostart_theme_fonts) {
					for (rule in prostart_theme_fonts[opt]) {
						if (opt+'_'+rule == id) {
							// Store new value in the color table
							prostart_customizer_update_theme_fonts(opt, rule, val);
							processed = true;
							break;
						}
					}
			...
			// Refresh CSS
			if (processed) prostart_customizer_update_css();
		}
	...
	// Actions buttons
	function prostart_customizer_actions(e) {
	...
	function prostart_customizer_update_theme_fonts(opt, rule, value) {
		prostart_theme_fonts[opt][rule] = value;
	}
	
	// Change theme fonts options if load fonts is changed
	function prostart_customizer_update_load_fonts() {
		var opt_list=[], i, tag, sel, opt, name='', family='', val='', new_val = '', sel_idx = 0;
		updateCSS = false;
		for (i=1; i <= prostart_customizer_vars['max_load_fonts']; i++) {
			name = api('load_fonts-'+i+'-name')();
			if (name == '') continue;
			family = api('load_fonts-'+i+'-family')();
			opt_list.push([name, family]);
		}
		for (tag in prostart_theme_fonts) {
	...
	function prostart_customizer_update_css() {
		...
		var fonts = prostart_customizer_add_theme_fonts(prostart_theme_fonts);

		// Make styles and add into css
		css += prostart_customizer_prepare_html_value(cssTemplate['theme_fonts']( fonts ));

		// Add colors
		for (var scheme in prostart_color_schemes) {
			
			var i, colors = [];
			
			// Copy all colors!
			for (i in prostart_color_schemes[scheme].colors) {
				colors[i] = prostart_color_schemes[scheme].colors[i];
			}
			
			// Make theme specific colors and tints
			colors = prostart_customizer_add_theme_colors(colors);
            ...
            var tmpl = jQuery( '#tmpl-prostart-color-scheme-' + scheme ).html().trim();
            for (i in colors) {
	...
	function prostart_customizer_add_theme_colors(colors) {
		if (prostart_additional_colors) {
			var clr = '', v = '';
			for (var k in prostart_additional_colors) {
				v = prostart_additional_colors[k];
				clr = colors[v['color']];
		...
		for (var tag in fonts) {
			...
			rez[tag+'_font-size'] = typeof fonts[tag]['font-size'] != 'undefined' 
									&& fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + prostart_customizer_prepare_css_value(fonts[tag]['font-size']) + ";"
												: '';
			...
			rez[tag+'_margin-top'] = typeof fonts[tag]['margin-top'] != 'undefined' 
									...
									&& fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + prostart_customizer_prepare_css_value(fonts[tag]['margin-top']) + ";"
												: '';
			rez[tag+'_margin-bottom'] = typeof fonts[tag]['margin-bottom'] != 'undefined' 
									&& fonts[tag]['margin-bottom'] != '' 
									&& fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + prostart_customizer_prepare_css_value(fonts[tag]['margin-bottom']) + ";"
												: '';
		}
		return rez;
	}
	...
	// Add ed to css value
	function prostart_customizer_prepare_css_value(val) {
		if (val != '' && val != 'inherit') {
			var ed = val.substr(-1);
			if ('0'<=ed && ed<='9') val += 'px';
		}
	...
	function prostart_customizer_prepare_html_value(val) {
