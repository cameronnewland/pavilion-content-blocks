...
?>><?php
    ...
    // Post meta on featured image (catego)
	...
	// Featured image
	if ( prostart_is_off(prostart_get_theme_option('hide_featured_on_single'))
			&& !prostart_sc_layouts_showed('featured') 
		...
		prostart_show_post_featured(array(
		        'post_info' => '<div class="post_info_top">' . '<div class="post_categories_block">'. (!empty($res['categories']) ? ($res['categories']) : '') .'</div>' . '</div>'
            )
        );
	...
	// Title and post meta
	if ( (!prostart_sc_layouts_showed('title') || !prostart_sc_layouts_showed('postmeta')) && !in_array(get_post_format(), array('link', 'aside', 'status', 'quote')) ) {
		do_action('prostart_action_before_post_title'); 
		?>
		<div class="post_header entry-header">
			<?php
			// Post title
			if (!prostart_sc_layouts_showed('title')) {
				the_title( '<h3 class="post_title entry-title"'.($prostart_seo ? ' itemprop="headline"' : '').'>', '</h3>' );
			}
			// Post meta
			if (!prostart_sc_layouts_showed('postmeta') && prostart_is_on(prostart_get_theme_option('show_post_meta'))) {
		...
		<?php
		do_action('prostart_action_after_post_title'); 
	}
	...
	<div class="post_content entry-content" itemprop="mainEntityOfPage">
		...
		do_action('prostart_action_before_post_pagination'); 
		...
		if ( is_single() && !is_attachment() ) {
			...
			?><div class="post_meta post_meta_single"><?php
				...
				// Share
				if (prostart_is_on(prostart_get_theme_option('show_share_links'))) {
